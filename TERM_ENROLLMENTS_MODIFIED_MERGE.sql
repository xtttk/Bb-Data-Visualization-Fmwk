/* Oracle. Start Prep: Term Enrollments Added or Modified. Supporting visualization tool for system admins monitoring automated enrollment creation processes. Counts of enrollments added to a given DSK including Row and Availability status.
*/

SELECT 
  'The Enrollment DSK <b>'||TERM_ENROLLMENT_DSK||'</b><ul><li> Was modified on '||TERM_ENROLLMENTS_MODIFIED||'.</li><li>'||ENABLED_COUNT||' enrollments are ENABLED/AVAILABLE in the DSK.</li><li>'||DISABLED_COUNT||' enrollments are DISABLED in the DSK.</li><li>'||UNAVAILABLE_COUNT||' enrollments are UNAVAILABLE in the DSK.</li></ul><br>' AS MESSAGE, TERM_COUNT AS THE_COUNT
FROM
(SELECT 
